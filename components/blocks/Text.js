...
export default function Text({ block, visual, locale }) {
  ...
  return (
    ...
    >
      <div className={`${alignClass} container`}>
        <h2
        ...
        <div className={`${alignClass}`}>
          ...
          {block.link && (
            <div className="mt-8">
              <a
                className="inline-flex rounded text-button bg-button-back uppercase tracking-wide transition-opacity hover:opacity-80 text-sm py-4 px-6"
                href={block.link.url}
                target="_blank"
