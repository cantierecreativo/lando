...
import { OrganizationRecord, Site, VisualStyleRecord } from "@/gql/graphql";
...
export default function Footer(props: FooterProps) {
  const { org, visual, site, locale } = props;
  const alignClass = org?.newsletterFormUrl ? "" : "justify-center text-center";
  const srcLogo = visual?.logoRev?.url || null;
    ...
    <footer id="footer" className="container py-6 ">
      <div className={`${alignClass} md:flex gap-16`}>
        <div className="flex-1 my-12">
          ...
          {org?.streetAddress && (
            <div className="my-6">{renderHTML(org.streetAddress)}</div>
          )}
          {org?.phoneNumber && (
          ...
          {(org?.facebookUrl || org?.instagramUrl) && (
              ...
              {org.facebookUrl && (
                <li>
                  <a
                    ...
                    title={`${t("linkExternal", locale)} Facebook`}
                  >
                    <svg
              ...
              {org?.instagramUrl && (
                <li>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer noopener"
                    title={`${t("linkExternal", locale)} Instagram`}
                  >
                    <svg
                      className="w-6 h-6 fill-current"
                      viewBox="0 0 24 24"
        ...
        {org?.newsletterFormUrl && (
          <section className="flex-1 my-12">
            ...
            <SignupForm url={org?.newsletterFormUrl} locale={locale} />
          </section>
        )}
      </div>
      <div className="text-center py-6 text-sm">
        <a
          ...
          target="_blank"
          rel="noreferrer noopener"
          title={`${t("linkExternal", locale)} Cantiere Creativo`}
        >
