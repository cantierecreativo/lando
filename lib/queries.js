...
const carouselImage = `
  responsiveImage(sizes: "(min-width:768px) 45vw, 100vw", imgixParams: {auto: [format, compress], fit: max, w: 750}){
    ...imgFrag
  }
`;
...
query site($locale: SiteLocale!) {
  ...
  itinerariesIndex (locale: $locale) {
    model: _modelApiKey
    title
    id
  }
...
export const getArtworksIndex = `
query artworksIndex($locale: SiteLocale!) {
  page: artworksIndex(locale: $locale) {
    ${forAllPages}
    textHero
  }
  items: allArtworks(locale: $locale, first: 100, filter: {slug: {neq: null}}) {
    ${previewArtworks}
  }
}
${imgFrag}
...
export const getAllSlugsArtworks = `
query allArtworks ($locale: SiteLocale!){
  allArtworks (locale: $locale, first: 100, filter: {slug: {neq: null}}) {
    slug
  }
}
`;

export const getAllSlugsVideo = `
query allVideos ($locale: SiteLocale!){
  allVideos (locale: $locale, first: 100, filter: {slug: {neq: null}}) {
    slug
  }
}
`;
...
query ticketsPage($locale: SiteLocale!){
  ticketsPage(locale: $locale) {
    ${forAllPages}
    ${ticketsTemplate}
  }
...
query artwork($slug: String!, $locale: SiteLocale!){
  artwork(filter: {slug: {eq: $slug}}, locale: $locale) {
    ${forAllPages}
    ${detailTemplate}
    author
    year
...
query video($slug: String!, $locale: SiteLocale!){
...
query news($slug: String!, $locale: SiteLocale!){
  ...
  allNews(filter: {slug: {neq: $slug}}, first: "3", locale: $locale) {
    ${previewNews}
  }
}
${imgFrag}
